...
bool CrossPointSettings::saveToFile() const {
  ...
  serialization::writePod(outputFile, doubleTapPwrBtn);
  outputFile.close();
...
bool CrossPointSettings::loadFromFile() {
  ...
  do {
    ...
    if (++settingsRead >= fileSettingsCount) break;
    readAndValidate(inputFile, doubleTapPwrBtn, DOUBLE_TAP_PWRBTN_COUNT);
    if (++settingsRead >= fileSettingsCount) break;
  } while (false);

  inputFile.close();

  // Legacy statusBar value 3+ was FULL_BOOK (removed); coerce to FULL.
  if (statusBar >= STATUS_BAR_MODE_COUNT) {
