...
void TxtReaderActivity::renderStatusBar(const int orientedMarginRight, const int orientedMarginBottom,
  ...
  if (showBattery) {
    ScreenComponents::drawBattery(renderer, orientedMarginLeft, textY, showBatteryPercentage, !SETTINGS.readerDarkMode);
  }

  if (showTitle) {
    const int titleMarginLeft = 50 + 30 + orientedMarginLeft;
    int titleMarginRight = progressTextWidth + (showProgressText ? 30 : 0) + orientedMarginRight;
